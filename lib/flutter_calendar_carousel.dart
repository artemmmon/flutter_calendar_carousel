...
class _CalendarState<T extends EventInterface> extends State<CalendarCarousel<T>> {
  ...
  initState() {
    ...
    maxDate =
        widget.maxSelectedDate ?? DateTime(DateTime.now().year + 1, DateTime.now().month, DateTime.now().day);
    ...
    if (widget.weekFormat) {
      ...
      for (int _cnt = 0; 0 > minDate.add(Duration(days: 7 * _cnt)).difference(_targetDate).inDays; _cnt++) {
      ...
      child: GestureDetector(
        ...
        child: FlatButton(
          color: isSelectedDay && widget.selectedDayButtonColor != null
              ...
              : isToday && widget.todayButtonColor != null ? widget.todayButtonColor : widget.dayButtonColor,
  ...
  Future<Null> _selectDateFromPicker() async {
    ...
    if (selected != null) {
      ...
      if (widget.onDayPressed != null)
        widget.onDayPressed(
            selected, widget.markedDatesMap != null ? widget.markedDatesMap.getEvents(selected) : []);
    }
  }
  ...
  void _setDatesAndWeeks() {
    ...
    _endWeekday =
        DateTime(date[currentDateIndex].year, date[currentDateIndex].month + 1, 1).weekday - firstDayOfWeek;
    this._dates = date;
    this._weeks = week;
    ...
    if (page == -1) {
      ...
      if (widget.onCalendarChanged != null) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          widget.onCalendarChanged(!widget.weekFormat ? this._dates[page] : this._weeks[page][firstDayOfWeek]);
        });
      }
    }
  }
  ...
  TextStyle getDefaultDayStyle(
    ...
    return !isSelectable
        ...
        : (_localeDate.dateSymbols.WEEKENDRANGE.contains((index - 1 + firstDayOfWeek) % 7)) &&
            ...
            : isToday ? defaultTodayTextStyle : isSelectable && textStyle != null ? textStyle : defaultTextStyle;
  }
